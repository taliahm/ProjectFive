...
giftApp.getLcboProductReturn = function (userInput) {
...
giftApp.getLcboProductReturnThree = function (firstArrayReturn, userInput) {
	giftApp.getAlcohol = $.ajax({
		url: 'https://proxy.hackeryou.com',
		dataType: 'json',
		method: 'GET',
...
giftApp.getLcboProductReturnTwo = function (firstArrayReturn, thirdArrayReturn, userInput) {
	giftApp.getAlcoholTwo = $.ajax({
		...
		var secondArrayReturn = alcoholDataTwo.result;
		var allResults = [].concat(_toConsumableArray(firstArrayReturn), _toConsumableArray(secondArrayReturn), _toConsumableArray(thirdArrayReturn));
		var combinedAlcoholArray = allResults.filter(function (item) {
...
giftApp.getLcboStoresTwo = function (id, firstResult) {
	giftApp.lcboStorebyIdTwo = $.ajax({
		...
		var lcboStoresTwo = dataTwo.result;
		var lcboStoresTogether = [].concat(_toConsumableArray(firstResult), _toConsumableArray(lcboStoresTwo));
...
giftApp.getLcboStoresThree = function (id, togetherResult) {
	giftApp.lcboStorebyIdThree = $.ajax({
		...
		var lcboStoresThree = dataThree.result;
		var lcboStoresTogetherAgain = [].concat(_toConsumableArray(togetherResult), _toConsumableArray(lcboStoresThree));
...
giftApp.getLcboStoresFour = function (id, togetherResultAgain) {
	giftApp.lcboStorebyIdFour = $.ajax({
		...
		var lcboStoresFour = dataFour.result;
		var lcboStoresAlmostDone = [].concat(_toConsumableArray(togetherResultAgain), _toConsumableArray(lcboStoresFour));
...
giftApp.getLcboStoresFive = function (id, togetherResultAgainFinal) {
	giftApp.lcboStorebyIdFive = $.ajax({
		...
		var lcboStoresFive = dataFive.result;
		var lcboStoresFinal = [].concat(_toConsumableArray(togetherResultAgainFinal), _toConsumableArray(lcboStoresFive));
...
giftApp.convertStores = function (array) {
...
giftApp.filterByPrimeCat = function (array) {
	var idofEl = '#' + giftApp.userAlcoholChoice;
	var filterParamSelector = $('#alcoholType').children(idofEl);
	var filterParam = filterParamSelector.attr('data-filterParam');
	var arrayByName = array.filter(function (element) {
...
giftApp.userLocationManual = function () {
	var manualLocationEl = '<form id="locationField">\n                        <input id="autocomplete" placeholder="Enter your address" type="text"></input>\n                        <input id="submitLocation" value="find" type="submit"></input>\n                   </form>';
	var manualLocation = $('<div class="userLocationOverlay">').append(manualLocationEl);
...
giftApp.setManualMarker = function () {
	...
	var userImage = '/ProjectFive/assets/mapMarkerUser.png';
...
//EVENTS
giftApp.getUserChoice = function () {
...
giftApp.userChooseAgain = function () {
...
giftApp.closeZoomWindow = function () {
...
giftApp.filterByBudget = function (finalArray) {
	if (giftApp.userBudget === 'low') {
		var finalBudgetArray = finalArray.filter(function (element) {
		...
		var _finalBudgetArray = finalArray.filter(function (element) {
			return element.price_in_cents > 2001 && element.price_in_cents < 4000;
		});
		giftApp.sortedArray(_finalBudgetArray);
		// console.log('medium of budgets', finalBudgetArray);
	} else if (giftApp.userBudget === 'high') {
		var _finalBudgetArray2 = finalArray.filter(function (element) {
			return element.price_in_cents > 4001;
		});
		giftApp.sortedArray(_finalBudgetArray2);
		// console.log('highest of budgets', finalBudgetArray);
	}
};

giftApp.sortedArray = function (passedData) {
	var sortedByAbv = _.sortBy(passedData, 'alcohol_content');
	var arrayHalfLength = Math.floor(sortedByAbv.length / 2);
	if (giftApp.stressLevel === false) {
		var halfArray = sortedByAbv.slice(0, arrayHalfLength);
		giftApp.getFinalArray(halfArray);
	} else {
		var _halfArray = sortedByAbv.slice(arrayHalfLength, sortedByAbv.length);
		giftApp.getFinalArray(_halfArray);
	}
};

giftApp.getFinalArray = function (array) {
	var randomArray = _.shuffle(array);
	var finalArray = randomArray.slice(0, 6);
	giftApp.displayAlcohol(finalArray);
};

//Display results on page
giftApp.displayAlcohol = function (array) {
	$('.alcoholResults').show();
	...
	var elemArray = array.forEach(function (item) {
		...
		var allElems = $('<div class="resultItem">').append(elemString);
		$('.resultsShow').append(allElems);
	});
};

giftApp.getStressOfOccasion = function (param) {
	var filteredOccasion = giftApp.occasions.filter(function (item) {
		return item.occasion === param;
	});
	var truthie = filteredOccasion.map(function (item) {
		return item.stressLevel;
	});
	giftApp.stressLevel = truthie[0];
};

giftApp.smoothScroll = function (targetElem) {
	targetElem = $('.' + targetElem);
	$('html,body').animate({
		scrollTop: targetElem.offset().top }, 'slow');
};

giftApp.events = function () {
	giftApp.infoText();
